# Acceptance Checklist

**Course:** Mastering Semiconductor Key Modules for Enhanced CMOS Process Integration and Device Performance<br>
**Course code:** TGS-2024051248<br>
**Package:** v9.0 · 20 August 2026<br>
**Activity:** 3 — Balance Implantation and Thermal Budget<br>
**Alignment:** LO1 · K1, A2, A3<br>
**Duration:** 60 minutes


## Output Quality

- [ ] The recommendation distinguishes implant dose/energy, damage and activation mechanisms.
- [ ] Thermal-budget effects on activation and diffusion are explicit.
- [ ] Follow-up measurements can confirm or reject the proposed cause.
- [ ] Every claim is linked to a slide, workbook result, measurement or activity observation.
- [ ] Control limits and specification limits are not used interchangeably.
- [ ] The final decision includes an accountable follow-up action.
- [ ] File names use the required `A03-` prefix.

## Peer Review

**Reviewer:** ______________________________

**One strength:**


**One correction needed:**


**Learner confirmation after correction:** ______________________________
