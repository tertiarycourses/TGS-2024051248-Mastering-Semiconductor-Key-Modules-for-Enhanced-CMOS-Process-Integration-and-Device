# Learner Worksheet

**Course:** Mastering Semiconductor Key Modules for Enhanced CMOS Process Integration and Device Performance  
**Course code:** TGS-2024051248  
**Package:** v8.0 · 20 August 2026  
**Activity:** 6 — Analyse Yield and Process Capability  
**Alignment:** LO2 · K2, A3, A4  
**Duration:** 75 minutes


## Scenario Restatement

In your own words, describe the process-integration decision that this activity requires.

**Response:**



## Process and Evidence Table

| Stage / issue | Intended function | Input or risk | Measurement / evidence | Decision or follow-up |
|---|---|---|---|---|
| 1 |  |  |  |  |
| 2 |  |  |  |  |
| 3 |  |  |  |  |
| 4 |  |  |  |  |
| 5 |  |  |  |  |

## Analysis Questions

### Question 1

Why can Cp be acceptable while Cpk is unacceptable?

**Response:**





### Question 2

Why does a capability index not prove that the process is statistically stable?

**Response:**





## Decision

Select one: **Release / Conditional Release / Hold / Scrap / Proceed / Defer**

**Evidence-based rationale:**



**Next action, owner and due date:**
